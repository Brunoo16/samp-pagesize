...
static void Thread()
{
	...
	VirtualProtect(reinterpret_cast<void*>(offsets[0]), 1, PAGE_EXECUTE_READWRITE, &OldProtect);
	VirtualProtect(reinterpret_cast<void*>(offsets[1]), 1, PAGE_EXECUTE_READWRITE, &OldProtect);
	VirtualProtect(reinterpret_cast<void*>(offsets[2]), 4, PAGE_EXECUTE_READWRITE, &OldProtect);

	*reinterpret_cast<BYTE*>(offsets[0]) = 0x1;
	*reinterpret_cast<BYTE*>(offsets[1]) = 0x1;
	*reinterpret_cast<DWORD*>(offsets[2]) = reinterpret_cast<DWORD>(&pagesize_string);
}
